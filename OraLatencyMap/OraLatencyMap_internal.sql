...
--
-- See also credits, comments and change list in README.txt
--
...
declare
  ...
  begin
     ...
     v_line := v_line||'OraLatencyMap v1.1b Aug 2013 - Luca.Canali@cern.ch';   
  ...
  procedure print_heat_map(p_table t_integertable, p_rows pls_integer, p_cols pls_integer, 
                           p_palette_type varchar2, p_graph_header varchar2, p_graph_unit varchar2)
  is
  ...
  begin
     ...
     -- compute max value of v_delta_awr_data used further on to compute the color coding scale
    ...
    print_to_screen(v_line);
    v_line := asciiescape_backtonormal()||' (ms)';
    v_line := v_line||lpad(' ',p_rows+4,' ')||p_graph_unit;
    print_to_screen(v_line);

    -- print body of the heat map graph
    for y in 0..p_cols loop
    ...
    v_line := v_line||'. Max sum: '||to_char(v_max_col_sum);
  ...
  begin
    ...
    print_to_screen(v_line);
    print_to_screen('Top graph: x=time, y=latency bucket, color=N# of wait events per second. Palette=six shades of blue.');
    print_to_screen('Bottom graph: x=time, y=latency bucket, color=estimated time waited per sec. Palette=yellow-to-red.');
...
begin
    ...
    print_heat_map (g_table_wait_count,gc_num_rows,gc_num_bins,'blue','Number of waits (N#) per second','(N#/sec)'); 

    -- print heat map of estimated time waited per latency bucket. 
    -- Yellow-red palette (color is 3rd dimension to the plot)
    print_heat_map (g_table_time_waited,gc_num_rows,gc_num_bins,'red','Time waited (ms) per second','(ms/sec)'); 
