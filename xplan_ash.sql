...
set linesize 800 pagesize 0 tab off feedback off sqlterminator on underline on
...
begin
  execute immediate '
  select
          max(to_char(oldest_sample_time, ''YYYYMMDD HH24:MI:SS''))
  from
          gv$ash_info
  where
...
select /* XPLAN_ASH GET_SESSION_SQL */ /*+ NO_STATEMENT_QUEUING */
...
select  /* XPLAN_ASH CHILD_ADDRESS_TO_CHILD_NUMBER */ /*+ NO_STATEMENT_QUEUING */
...
select /* XPLAN_ASH GET_SESSION_DETAILS */ /*+ NO_STATEMENT_QUEUING */
...
select /* XPLAN_ASH IDENTIFY_SECOND_ID */ /*+ NO_STATEMENT_QUEUING */
...
select  /* XPLAN_ASH CHILD_NUMBER_TO_CHILD_ADDRESS */ /*+ NO_STATEMENT_QUEUING */
...
select /* XPLAN_ASH SEARCH_LAST_EXECUTION */ /*+ NO_STATEMENT_QUEUING */
...
select  /* XPLAN_ASH DEFINE_ASH_SAMPLES */ /*+ NO_STATEMENT_QUEUING */
        pred1
      , pred2
      , ln_pred1
      , ln_pred2
...
(
  select
        ...
        , px_servers_requested
        , px_servers_allocated
        , rpad('&gc3', round(greatest(case when duration < elapsed_time then duration / nullif(elapsed_time, 0) else 1 end * (&wgs - 1), 1)), '&gc3') || chr(10) ||
          rpad('&gc', round((cpu_time / nullif(elapsed_time, 0)) * case when elapsed_time < duration then elapsed_time / nullif(duration, 0) else 1 end * (&wgs - 1)), '&gc') ||
          rpad('&gc2', round(((user_io_wait_time + concurrency_wait_time + cluster_wait_time + application_wait_time + coalesce(queuing_time, 0)) / nullif(elapsed_time, 0)) * case when elapsed_time < duration then elapsed_time / nullif(duration, 0) else 1 end * (&wgs - 1)), '&gc2') ||
          rpad('&gc3', round((other_time / nullif(elapsed_time, 0)) * case when elapsed_time < duration then elapsed_time / nullif(duration, 0) else 1 end * (&wgs - 1)), '&gc3')
          as time_graph
...
column median_as                        heading "MEDIAN AS"
column stddev_as                        heading "AVERAGE AS|STDDEV"
column max_as                           heading "AVERAGE AS|MAX"
column min_as                           heading "AVERAGE AS|ACTIVE MIN"
...
(
  select
        ...
        , max(active_sessions) as max_as
        , coalesce(min(case when active_sessions > 0 then active_sessions end), 0) as min_as
  from
          base_data_g
),
...
select  /* XPLAN_ASH SQL_STATEMENT_EXECUTION_ASH_SUMMARY SQL_ID: &si */ /*+ NO_STATEMENT_QUEUING */
        instance_count
      , inst_count
      , session_id
      , a.instance_id
...
&_IF_ORA11_OR_HIGHER        , top_level_sql_id
      ...
      , case when max_as >= &rnd_thr then round(max_as) else round(max_as, 1) end as max_as
      , case when min_as >= &rnd_thr then round(min_as) else round(min_as, 1) end as min_as
...
select  /* XPLAN_ASH DETERMINE_DUPLICATOR SQL_ID: &si */ /*+ optimizer_features_enable('11.1.0.7') NO_STATEMENT_QUEUING */
        duplicator
      , dist_sample_count
from
        dist_count
...
select  /* XPLAN_ASH SQL_STATEMENT_EXECUTION_ASH_SUMMARY_CROSS_INSTANCE SQL_ID: &si */ /*+ NO_STATEMENT_QUEUING */
        a.instance_id
      , first_sample
      , last_sample
      , start_active
...
select  /* XPLAN_ASH ASH_SESSION_SUMMARY_OTHER_ACTIVITY SQL_ID: &si */ /*+ NO_STATEMENT_QUEUING */
...
select  /* XPLAN_ASH ASH_SESSION_DETAILS_OTHER_ACTIVITY SQL_ID: &si */ /*+ NO_STATEMENT_QUEUING */
        nullif(a.instance_id, -1)                                                                                                                       as instance_id
      , a.sql_id
...
select  /* XPLAN_ASH GLOBAL_ASH_SUMMARY_CONCURRENT_EXECUTION SQL_ID: &si */ /*+ NO_STATEMENT_QUEUING */
        nullif(a.instance_id, -1)                                                         as instance_id
      , sample_count
      , cpu_sample_count
      , round(cpu_sample_count / sample_count * 100)                                      as percentage_cpu
...
select  /* XPLAN_ASH CONCURRENT_ACTIVITY_CONCURRENT_EXECUTION SQL_ID: &si */ /*+ NO_STATEMENT_QUEUING */
      ...
      , round(min(time_waited) / 1000, 1)                                           as min_tim_wait_ms
      , round(max(time_waited) / 1000, 1)                                           as max_tim_wait_ms
      , count(*)                                                                    as sample_count
...
column min_tim_wait_ms clear
column max_tim_wait_ms clear
...
select  /* XPLAN_ASH ASH_SQLID_DETAILS_CONCURRENT_ACTIVITY SQL_ID: &si */ /*+ NO_STATEMENT_QUEUING */
        instance_id
      , sql_id
      , sql_text
&_IF_ORA11_OR_HIGHER       , top_level_sql_id
...
select  /* XPLAN_ASH IO_SUMMARY SQL_ID: &si */ /*+ NO_STATEMENT_QUEUING */
        instance_id
      , duration_secs_t
      , cast(nullif('+' || to_char(extract(day from numtodsinterval(duration_secs_t, 'SECOND')), 'TM') || ' ' || substr(to_char(numtodsinterval(duration_secs_t, 'SECOND')), 12, 8), '+ ') as varchar2(12)) as duration_t
...
&THIS_STATEMENT                                   , round(((max(sample_time) - min_sql_exec_start) * 86400)) + &sample_freq   as duration_secs_t
&CONCU_ACTIVITY                                   , round(((max(sample_time) - min(sample_time)) * 86400)) + &sample_freq   as duration_secs_t
...
&_IF_ORA11_OR_HIGHER                                                       , sql_exec_id
                                                ...
                                                from
                                                        &global_ash ash
                                                where
&THIS_STATEMENT                                                        sql_id = '&si'
&THIS_STATEMENT &_IF_ORA11_OR_HIGHER                                                          and ((sql_exec_start = to_date('&ls', '&dm') and sql_exec_id = &li) or (sql_exec_start is null and sql_exec_id is null))
&THIS_STATEMENT                                                and     &ash_pred1 &ash_pred2
&THIS_STATEMENT                                                and     cast(sample_time as date) between to_date('&ash_min_sample_time', 'YYYY-MM-DD HH24:MI:SS') and to_date('&ash_max_sample_time', 'YYYY-MM-DD HH24:MI:SS')
-- For versions before 11g, consider the event "PX Deq Credit: send blkd" to be idle (as it is done from 11g on)
&THIS_STATEMENT &_IF_LOWER_THAN_ORA11 &ignore_PX_credit_blkd_10g                                                and     (event is null or event != 'PX Deq Credit: send blkd')
&CONCU_ACTIVITY                                                        (&ash_ln_pred1 &ash_ln_pred2)
&CONCU_ACTIVITY                                                and     instr('&op', 'ASH') > 0
&CONCU_ACTIVITY                                                and     cast(sample_time as date) between to_date('&ash_min_sample_time', 'YYYY-MM-DD HH24:MI:SS') and to_date('&ash_max_sample_time', 'YYYY-MM-DD HH24:MI:SS')
-- For versions before 11g, consider the event "PX Deq Credit: send blkd" to be idle (as it is done from 11g on)
&CONCU_ACTIVITY &_IF_LOWER_THAN_ORA11 &ignore_PX_credit_blkd_10g  and     (event is null or event != 'PX Deq Credit: send blkd')
&CONCU_ACTIVITY                                                and     &inst_id in
&CONCU_ACTIVITY                                                (
&CONCU_ACTIVITY                                                  select
&CONCU_ACTIVITY                                                          distinct
&CONCU_ACTIVITY                                                          &inst_id
&CONCU_ACTIVITY                                                  from
&CONCU_ACTIVITY                                                          &global_ash ash
&CONCU_ACTIVITY                                                  where
&CONCU_ACTIVITY                                                          sql_id = '&si'
&CONCU_ACTIVITY &_IF_ORA11_OR_HIGHER                             and ((sql_exec_start = to_date('&ls', '&dm') and sql_exec_id = &li) or (sql_exec_start is null and sql_exec_id is null))
&CONCU_ACTIVITY                                                  and     &ash_pred1 &ash_pred2
&CONCU_ACTIVITY                                                  and     cast(sample_time as date) between to_date('&ash_min_sample_time', 'YYYY-MM-DD HH24:MI:SS') and to_date('&ash_max_sample_time', 'YYYY-MM-DD HH24:MI:SS')
-- For versions before 11g, consider the event "PX Deq Credit: send blkd" to be idle (as it is done from 11g on)
&CONCU_ACTIVITY &_IF_LOWER_THAN_ORA11 &ignore_PX_credit_blkd_10g            and     (event is null or event != 'PX Deq Credit: send blkd')
&CONCU_ACTIVITY                                                )
...
---------
select
        'Concurrent activity I/O Summary based on ASH' as message
from
        dual
where
        instr('&op', 'ASH') > 0
...
---------
select
        'Concurrent activity I/O Summary per Instance ASH' as message
from
        dual
where
        instr('&op', 'ASH') > 0
...
undefine THIS_STATEMENT
undefine CONCU_ACTIVITY
...
select  /* XPLAN_ASH PARALLEL_DEGREE_INFO SQL_ID: &si */ /*+ NO_STATEMENT_QUEUING */
        nullif(a.instance_id, -1) as instance_id
      , a.dfo
      , start_active
      , duration_secs_t
...
select  /* XPLAN_ASH ACTIVITY_SUMMARY SQL_ID: &si */ /*+ NO_STATEMENT_QUEUING */
        &GROUP_CROSS_INSTANCE as instance_id
...
(
  select
        ...
        , greatest(sum(diff_aas), 0) as skew_plan_line_weighted
        --, min(dop) as dop
        , max(dop) as dop
...
--&_IF_LOWER_THAN_ORA112           , ltrim(extract(xmlagg(xmlelement("V", rpad('&gc3', case when proc_cnt = 1 or ratio > avg_ratio + avg_ratio / 10 then round((ratio - case when proc_cnt = 1 then 0 else /*avg_ratio*/ 0 end) * &pgs) end, '&gc3')) order by rn), '/V/text()'), ',') as procs_graph
          ...
          , rpad('&gc3', round(case when coalesce(max(dist_dop), 1) > 1 or coalesce(max(dop), 1) <= 1 then greatest(least(max(least(skew_plan_line, samples_active) / nullif(total_samples_active, 0)), 1), least(sum(diff / rnk / nullif(samples_active, 0) * (samples_active / nullif(total_samples_active, 0))), 1)) else least(max(least(skew_plan_line_weighted, samples_active * dop) / nullif(total_samples_active * dop, 0)), 1) end * &pgs), '&gc3') as procs_graph
...
select  /* XPLAN_ASH ACTIVITY_PLAN_LINE SQL_ID: &si */ /*+ NO_STATEMENT_QUEUING */
        plan_table_output
      , plan_table_count
from
        xplan_data
...
column io_read            format a8         heading "ReadB"                           &_SHOW_IO_READ.print
column io_write           format a8         heading "WriteB"                          &_SHOW_IO_WRITE.print
