...
-- uncommet lines in below description to get all executions....
-- Comment the below line if you want all exec(including the running one..this will be 0) 

prompt
prompt
prompt Comment lines in the script to get all execution
prompt including the one where execution=0
prompt
set echo off
set feedback off timing off verify off pagesize 100 recsep off
set serveroutput on size 1000000 format wrapped trimout on trimspool on
col phv heading "Plan|Hash Value"
col snap_time format a16 truncate heading "Snapshot|Time"
...
select	to_char(s.begin_interval_time, 'DY DD-MON HH24:MI') snap_time,
